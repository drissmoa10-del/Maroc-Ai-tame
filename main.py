...
# دالة توليد المحتوى مع حماية ضد أي خطأ في الرد
def generate_ai_text(prompt_text):
    ...
    try:
        ...
        # التحقق واكتشاف الخطأ إن وجد في رد جوجل
        if "candidates" in res_json and len(res_json["candidates"]) > 0:
            return res_json['candidates'][0]['content']['parts'][0]['text']
        else:
            logging.error(f"رد غير متوقع من جوجل: {res_json}")
            return f"⚠️ تنبيه من جوجل (قد يكون المحتوى محظوراً أو مفتاح API غير فعال): {res_json}"
