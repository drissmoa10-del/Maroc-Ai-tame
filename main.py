...
# دالة ذكية ومستقرة للاتصال بجيميناي مباشرة عبر HTTP API
def generate_gemini_content(prompt_text):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            error_msg = res_json.get('error', {}).get('message', 'خطأ غير معروف من جيميناي')
            return f"⚠️ خطأ من Google API: {error_msg}"
...
def handle_user_input(message):
    ...
    try:
        ...
        full_output = generate_gemini_content(prompt)
