...
def client_designer(sujet):
    try:
        ...
        response = model.generate_content(prompt)
        msg = f"🎨 *العميل 3 (المصمم): اقترح 3 صور جاهزة لترفق مع المقال*\n\n{response.text}\n\n👉 اضغط 'نشر' للانتقال لعميل السوشيال ميديا (فيسبوك وبنتريست)، أو 'لا' للإلغاء."
        bot.send_message(YOUR_CHAT_ID, msg, parse_mode="Markdown")
    except Exception as e:
        bot.send_message(YOUR_CHAT_ID, f"خطأ فالمصمم: {e}")

# ========== العميل 4: عميل السوشيال ميديا (النشر في فيسبوك وبنتريست) ==========
def client_social_publisher(sujet):
    global selected_social_post
    try:
        prompt = f"اكتب لي منشور ترويجي قصير وجذاب جداً يناسب منصات التواصل الاجتماعي (Facebook, Pinterest) للترويج لهذا المقال: {sujet} مع هاشتاغات قوية."
        response = model.generate_content(prompt)
        ...
        # 2. النشر في بنتريست (Pinterest API)
        if PINTEREST_TOKEN and PINTEREST_BOARD_ID:
            url_pin = "https://api.pinterest.com/v5/pins"
            headers = {"Authorization": f"Bearer {PINTEREST_TOKEN}", "Content-Type": "application/json"}
            payload_pin = {
                "board_id": PINTEREST_BOARD_ID,
                "title": sujet[:100],
                "description": selected_social_post
            }
            res_pin = requests.post(url_pin, json=payload_pin, headers=headers)
            if res_pin.status_code == 201:
                status_report += "\n✅ تم نشر البن पिन (Pin) في بنتريست بنجاح."
            else:
                status_report += f"\n❌ خطأ في بنتريست: {res_pin.text}"

        if not status_report:
            status_report = "\n📌 (وضع التحضير): المنشورات جاهزة للاستخدام بانتظار إعداد مفاتيح المنصات."

        msg = f"📢 *العميل 4 (النشر الاجتماعي - فيسبوك وبنتريست):*{status_report}\n\n{selected_social_post}\n\n👉 أرسل كلمة 'فيديو' للانتقال لعميل يوتيوب، أو 'لا' للإلغاء."
        bot.send_message(YOUR_CHAT_ID, msg, parse_mode="Markdown")
    except Exception as e:
...
def client_video_creator(sujet):
    try:
        ...
        yt_status = "✅ سكريبت الفيديو جاهز ومجدول للرفع على يوتيوب شورتس."
        if YT_API_KEY:
            yt_status += " (تم رصد مفتاح يوتيوب API بنجاح)."
...
def send_welcome(message):
    bot.reply_to(message, "🚀 مرحباً بك يا إدريس! النظام الخماسي الشامل (Pro) يعمل بكامل طاقته لنشر المحتوى في جميع منصاتك. جاري جلب أحدث الترندات...")
    client_chercheur()
