...
# متغيرات المنصات (بنتريست ويوتيوب)
...
def client_designer(sujet):
    try:
        ...
        response = model.generate_content(prompt)
        msg = f"🎨 *العميل 3 (المصمم): اقترح 3 صور جاهزة لترفق مع المقال*\n\n{response.text}\n\n👉 اضغط 'نشر' للانتقال لمنشور السوشيال ميديا وبنتريست، أو 'لا' للإلغاء."
        bot.send_message(YOUR_CHAT_ID, msg, parse_mode="Markdown")
    except Exception as e:
        bot.send_message(YOUR_CHAT_ID, f"خطأ فالمصمم: {e}")

# ========== العميل 4: عميل السوشيال ميديا (بنتريست والمنشورات) ==========
def client_social_publisher(sujet):
    global selected_social_post
    try:
        prompt = f"اكتب لي منشور ترويجي قصير وجذاب جداً يناسب منصات التواصل الاجتماعي (Pinterest حساب {PINTEREST_USERNAME}) للترويج لهذا المقال: {sujet} مع هاشتاغات قوية."
        response = model.generate_content(prompt)
        selected_social_post = response.text
        
        status_report = f"\n📌 حساب بنتريست: {PINTEREST_USERNAME}\nℹ️ (فيسبوك مؤقتاً يدوي حتى تسترجع الصفحة)."
...
def client_video_creator(sujet):
    try:
        ...
        yt_status = "\n✅ مفتاح يوتيوب API متصل وجاهز." if YT_API_KEY else "\n⚠️ (في انتظار إضافة YT_API_KEY في Railway)."

        msg = f"🎬 *العميل 5 (يوتيوب شورتس):*{yt_status}\n\n{response.text}\n\n🚀 *تمت دورة العمل بنجاح تام! صيفط (/start) للبدء من جديد.*"
        bot.send_message(YOUR_CHAT_ID, msg, parse_mode="Markdown")
    except Exception as e:
...
def send_welcome(message):
    bot.reply_to(message, "🚀 مرحباً بك يا إدريس! النظام الذكي يشتغل بكفاءة عالية. جاري جلب أحدث الترندات...")
    client_chercheur()
...
def handle_user_input(message):
    ...
    if text in ['1', '2', '3']:
        ...
        bot.send_message(YOUR_CHAT_ID, "📊 **Breakdown - الخطوة 4:** جاري إعداد محتوى السوشيال ميديا وبنتريست...")
...
print("النظام المتكامل خدام بنجاح... 🚀")
